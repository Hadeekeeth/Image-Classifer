...
def nn_setup(structure='densenet121',dropout=0.5, hidden_layer1 = 120,lr = 0.001,power='gpu'):
    ...
    if torch.cuda.is_available() and power == True:
        model.cuda()
...
def predict(image_path,model, topk=5,power='gpu'):
    ...
    probabiility = torch.exp(output).data
    top_probs,top_labels = probabiility.topk(topk)
    top_probs = top_probs.cpu().numpy()[0]
    top_labels=top_labels.cpu().numpy()[0]

    idx_to_class = {val: key for key, val in model.class_to_idx.items()}
    classes = [idx_to_class[x] for x in top_labels]
